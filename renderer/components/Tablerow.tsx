...
export default function TableRow({ row, onUpdate, onDelete }) {
  ...
  return (
    <tr className="hover:bg-slate-50 transition-colors duration-150">
      <td className="w-[34%] px-4 py-3 whitespace-nowrap text-sm text-gray-700 md:px-6">
        <MonthYearPicker
          value={row.monthYear}
          onChange={(updated) => onUpdate(row.id, "monthYear", updated)}
        />
      </td>
      <td className="w-[25%] px-4 py-3 whitespace-nowrap text-sm text-gray-700 md:px-6">
        <input
          type="number"
          name="income"
      ...
      <td className="w-[16%] px-4 py-3 whitespace-nowrap text-center text-sm font-medium md:px-6">
        <button
